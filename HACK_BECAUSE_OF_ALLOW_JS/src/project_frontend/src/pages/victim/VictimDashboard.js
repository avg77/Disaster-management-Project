...
const VictimDashboard = ({ currentUser }) => {
    ...
    return (<div className="victim-dashboard">
      ...
      <div className="dashboard-content">
        <div className="request-section">
          ...
          <form onSubmit={handleSubmitRequest} className="request-form">
            <div className="form-group">
              ...
              <select id="type" name="type" value={newRequest.type} onChange={handleInputChange} required disabled={loading} className="form-control">
            ...
            <div className="form-group">
              ...
              <input type="number" id="numberOfPeople" name="numberOfPeople" value={newRequest.numberOfPeople} onChange={handleInputChange} min="1" step="1" required disabled={loading} className="form-control" placeholder="Enter number of people"/>
            </div>

            <div className="form-group">
              <label htmlFor="description">Description:</label>
              <textarea id="description" name="description" value={newRequest.description} onChange={handleInputChange} placeholder="Please describe your needs in detail..." required disabled={loading}/>
            </div>

            <div className="form-group">
              <label htmlFor="urgency">Urgency Level:</label>
              <select id="urgency" name="urgency" value={newRequest.urgency} onChange={handleInputChange} required disabled={loading} className="form-control">
                {getUrgencyOptions(newRequest.type).map(option => (<option key={option.value} value={option.value}>
                    {option.label}
                  </option>))}
              </select>
            </div>
            ...
            <div className="form-group">
              ...
              <div className="location-input-group">
                <input type="text" id="location" name="location" value={newRequest.location} onChange={handleInputChange} placeholder="Your current location..." required disabled={loading || gpsLoading} className="form-control"/>
                <button type="button" onClick={getGPSLocation} disabled={loading || gpsLoading} className="gps-button">
                  {gpsLoading ? (<span className="loading-spinner"></span>) : ('📍 Get Location')}
                </button>
              </div>
            </div>
        ...
        <div className="active-requests">
          ...
          {activeRequests.length === 0 ? (<p className="no-requests">You don't have any active requests.</p>) : (<div className="requests-list">
              {activeRequests.map((request, index) => (<div key={index} className="request-card">
                  <div className="request-header">
                    ...
                    <div className="request-header-right">
                      ...
                      {request.status.toLowerCase() === 'pending' && (<button className="cancel-button" onClick={() => handleCancelRequest(request.timestamp)} disabled={cancellingRequestId === request.timestamp}>
                          {cancellingRequestId === request.timestamp ? (<span className="loading-spinner"></span>) : ('❌ Cancel')}
                        </button>)}
                    </div>
                  </div>
